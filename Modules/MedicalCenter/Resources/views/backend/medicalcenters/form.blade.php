    ...
    <div class="col-12 mb-3">
        <div class="form-group">
            ...
            @if(isset($$module_name_singular) && $$module_name_singular->image)
                <img src="{{ asset($$module_name_singular->image) }}" class="img-fluid mt-2" style="max-height: 200px;" alt="{{ $$module_name_singular->name }}">
            @endif
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-12 mb-3">
        <div class="form-group">
            ...
            ?>
            {{ html()->label($field_lable, $field_name)->class('form-label') }} {!! field_required($required) !!}
            {{ html()->textarea($field_name)->placeholder($field_placeholder)->class('form-control')->attributes(["$required", "rows" => 10]) }}
        </div>
    </div>
</div>
    ...
    $(document).ready(function() {
        ...
        $('#district').change(function() {
            ...
            if (district in subDistricts) {
                $.each(subDistricts[district], function(i, subDistrict) {
                    $subDistrict.append($('<option></option>').val(subDistrict).text(subDistrict));
                });
            }
            $subDistrict.trigger('change');
        });

        // Trigger change on page load if district is already selected (e.g., when editing)
