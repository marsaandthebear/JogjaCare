...
class UserController extends Controller
{
     ...
     * @return \Illuminate\View\View The redirect response if the user ID is not the same as the authenticated user's ID, otherwise the view with the change password form.
     */
    public function profileUpdate(Request $request)
    {
        ...
        if ($request->hasFile('avatar')) {
            if ($user->getMedia($this->module_name)->first()) {
                $user->getMedia($this->module_name)->first()->delete();
            }

            $media = $user->addMedia($request->file('avatar'))->toMediaCollection($this->module_name);
            $user->avatar = $media->getUrl();
        }

        $user->save();

        return redirect()->route('frontend.users.profile', $user->username)->with('flash_success', 'Profile updated successfully!');
    }
